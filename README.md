...
## Translation pipeline (i18n)

The site ships translated article content at `/<lang>/sustainability-news/<slug>` for each language in [`src/i18n.rs`](src/i18n.rs). Translations are generated by [`scripts/translate.py`](scripts/translate.py) — a Claude-backed pipeline with hash-based caching and a built-in quality scan.
...
```bash
# Bulk-translate everything that's new or changed (real-time path; 100% cost).
./scripts/translate.py

# Same thing via Batch API — 50% cheaper, results back within minutes-to-hours.
./scripts/translate.py --batch                   # submit
./scripts/translate.py --batch-poll <batch_id>   # download + auto-scan + auto-retry

# Scope to a single language or file.
./scripts/translate.py --lang es
./scripts/translate.py --file content/articles/foo.md --lang pt --force

# Standalone quality audit (line-overlap + non-ASCII heuristics).
# Auto-retries any suspect file with Haiku → Sonnet escalation.
./scripts/translate.py --audit            # all languages
./scripts/translate.py --audit --lang pt  # one language
```

**What it does:**

- Source of truth: English markdown at `content/articles/<slug>.md`.
- Output: translated markdown at `content/articles/<lang>/<slug>.md`. Mirror filenames so `build.rs` can pair them.
- Brand glossary: `Heartland`, `Imperium*`, `Detroit`, `Michigan`, etc. never get translated. Extend `GLOSSARY` in the script for new products.
- Hash cache (`.translation-cache.json`): unchanged source files are skipped on re-run. Commit the cache to git so CI inherits it.
- Prompt caching enabled: ~10× discount on the system prompt for the same language within a run.
- Quality scan after every batch poll: flags partial translations (>25% body lines verbatim from English source, or <0.5% non-ASCII in a >800-char body) and auto-retries them, escalating to Sonnet 4.6 when Haiku is stuck.

**Cost reference (Haiku 4.5 + Batch API):**

| Volume | Real-time | Batch |
|---|---|---|
| 178 articles × 1 language | ~$3 | ~$1.50 |
| 178 articles × 17 languages | ~$51 | **~$25** |
| Incremental (1 new article × 17 langs) | ~$0.20 | ~$0.10 |

**CI integration:** [`.github/workflows/translate.yml`](.github/workflows/translate.yml) runs `translate.py` on every push that touches `content/articles/*.md` and commits the resulting translations back to `main`. Requires `ANTHROPIC_API_KEY` repo secret.

**Adding a language:** add a `(code, "Name")` entry to `LANGUAGES` in `scripts/translate.py` AND to the `Language` enum + `Language::ALL` in [`src/i18n.rs`](src/i18n.rs). The list must stay in sync — there's no auto-generation between the two.

**Removing a translation surface:** delete the relevant `content/articles/<lang>/` directory + matching cache keys, push, Amplify rebuilds without it. The `LangArticle` route gracefully falls back to English when a translation is missing for a slug.
